...
Mario.LevelState.prototype.GetHappy = function(context) {
	...
	if(limit==1){
		if(Mario.MarioCharacter.GetFire()){
			...
			this.ProcessEvent("Oops!!", "No more Bullets", true, true, 2, "powerdown");
		}
		else{
			Mario.MarioCharacter.SetLarge(false, false);
			this.ProcessEvent("Hahaha!!", "Be an imp now", true, true, 2, "powerdown");
		}
	}
	
	if(limit==2){
		Mario.MarioCharacter.setSideWaysSpeed(4);
		msg1='You call that fast?';
		msg2='Let me show you';
		sound="pipe";
		this.ProcessEvent(msg1, msg2, true, true, 2, sound);
	}
	if(limit==3){
...
Mario.LevelState.prototype.GetSad = function(context) {
	...
	if(limit==1){
		if(!Mario.MarioCharacter.GetLarge()){
			Mario.MarioCharacter.SetLarge(true, false);
			this.ProcessEvent("Don't be sad", "You just got a boost", true, true, 2, "powerup");
		}
		else{
			Mario.MarioCharacter.SetLarge(true, true);
			this.ProcessEvent("Smile please", "Time for some firepower", true, true, 2, "powerup");
		}
	}
	else if(limit == 2 ){
		 ...
		 msg2='';
		 sound="hop";
		 this.ProcessEvent(msg1, msg2, true, false, 2, sound);
	}
	else if(limit == 3){
			coinBonus=true;
			if(coinBonus){
				for(i=0;i<20;i++){
				Mario.MarioCharacter.GetCoin();
				this.ProcessEvent("Congrats!!!", "You got 20 coins", true, false, 2, "coin");
				Enjine.Resources.PlaySound("coin");
			}
			coinBonus=false;
			}
...
Mario.LevelState.prototype.GetSurprised= function(context) {
	//msg=[["Surprise!!!","Huh! You found mario surprising"],["What just happened!!!","I don't want to live on this planet anymore"],["What is this??","Y u not know Mario, Heretic :("]];
	number=Math.floor(Math.random() * 3);

	duration = 3;
	sound ="kick";
	if(number === 1) {
		this.ProcessEvent("Surprise!!!", "Huh! You found mario surprising", true, false, 2, sound);
	}else if(number === 2) {
		this.ProcessEvent("What just happened!!!", "I don't want to live on this planet anymore", true, false, 2, sound);
	} else {
		this.ProcessEvent("What is this??", "Y u not know Mario, Heretic :(", true, false, 2, sound);
	}
};
